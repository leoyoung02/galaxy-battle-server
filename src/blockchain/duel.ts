...
import { ComplexAutData, DuelInfo, DuelInfoResponce, OpponentResponce, TelegramAuthData } from "./types.js";
...
export async function GetDuelData (duelId: string): Promise<DuelInfo | null> {
    return new Promise(async (resolve, reject) => {
        ...
        fetch(url).then(res => {
            if (res.status !== 200) reject(`Server responce is invalid, code ${res.status}`)
            return res.json();
